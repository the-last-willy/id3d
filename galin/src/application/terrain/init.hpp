...
void App::init() {
    ...
    { // Terrain.
        ...
        for(size_t j = 1; j < ny - 2; ++j) {
            ...
            auto right_dfdx = (at(terrain.heights, nx - 1, j) - at(terrain.heights, nx - 2, j)) / dx;
            auto left_dfdy = (at(terrain.heights, 0, j + 1) - at(terrain.heights, 0, j - 1)) / (2 * dy);
            auto right_dfdy = (at(terrain.heights, nx - 1, j + 1) - at(terrain.heights, nx - 1, j - 1)) / (2 * dy);
            at(terrain.gradients, 0, j) = agl::vec2(left_dfdx, left_dfdy);
            at(terrain.gradients, nx - 1, j) = agl::vec2(right_dfdx, right_dfdy);
        }
        for(size_t i = 1; i < nx - 2; ++i) {
            //replace 0 by domain's lower bound
            auto up_dfdx = (at(terrain.heights, i + 1, ny - 1) - at(terrain.heights, i - 1, ny - 1)) / (2 * dx);
            auto down_dfdx = (at(terrain.heights, i + 1, 0) - at(terrain.heights, i - 1, 0)) / (2 * dx);
            auto up_dfdy = (at(terrain.heights, 1, ny - 1) - at(terrain.heights, 0, ny - 2)) / dy;
            auto down_dfdy = (at(terrain.heights, i, 1) - at(terrain.heights, i, 0)) / dy;
            at(terrain.gradients, i, ny - 1) = agl::vec2(up_dfdx, up_dfdy);
        ...
        at(terrain.gradients, nx - 1, 0) = agl::vec2(
                                            (at(terrain.heights, nx - 1, 0) - at(terrain.heights, nx - 2, 0)) / dx,
                                            (at(terrain.heights, nx - 1, 1) - at(terrain.heights, nx - 1, 0)) / dy
                                        );
        at(terrain.gradients, 0, ny - 1) = agl::vec2(
                                            (at(terrain.heights, 1, ny - 1) - at(terrain.heights, 0, ny - 1)) / dx,
                                            (at(terrain.heights, 0, ny - 1) - at(terrain.heights, 0, ny - 2)) / dy
                                        );
        at(terrain.gradients, nx - 1, ny - 1) = agl::vec2(
                                            (at(terrain.heights, nx - 1, ny - 1) - at(terrain.heights, nx - 2, ny - 1)) / dx,
                                            (at(terrain.heights, nx - 1, ny - 1) - at(terrain.heights, nx - 1, ny - 2)) / dy
                                        );
