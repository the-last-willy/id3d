...
void update_laplacian(Terrain& t) {
    ...
    for(std::size_t i = 1; i < nx - 2; ++i) {
        for(std::size_t j = 1; j < ny; ++j) {
            ...
            l(i, j) = agl::length(agl::vec2(dfdx, dfdy));
        }
    }
    for(size_t j = 1; j < ny - 2; ++j) {
        ...
        l(0, j) = agl::length(agl::vec2(left_dfdx, left_dfdy));
        l(nx - 1, j) = agl::length(agl::vec2(right_dfdx, right_dfdy));
    }
    for(size_t i = 1; i < nx - 2; ++i) {
        ...
        l(i, ny - 1) = agl::length(agl::vec2(up_dfdx, up_dfdy));
        l(i, 0) = agl::length(agl::vec2(down_dfdx, down_dfdy));
    }
}
